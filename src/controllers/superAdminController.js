    ...
    if (sucursal.email !== email) {
      ...
      if (sucursalEmail) {
        const error = new Error(
          "Ya hay una sucursal con este email, intente otro",
        );
        return res.status(409).json({
          msg: error.message,
        });
...
const obtenerClientesPorSucursal = async (req, res) => {
  ...
  try {
    ...
    return res.json({
      data: {
        sucursal: {
          id: sucursal.id,
          name: sucursal.name,
          administradorSucursalId: sucursal.superAdminId,
        },
        clientesResumen: {
    ...
    console.log(
      `Hubo un error al obtener los clientes de la sucursal con id: ${idSucursal}`,
    );
    const error = new Error("Hubo un error en el servidor");
    return res.status(500).json({
      msg: error.message,
...
const perfilAdministrador = async (req, res) => {
  ...
  try {
    const administrador =
      await SuperAdmin.scope("eliminarInfo").findByPk(idAdministrador);
...
const editarPerfilAdministrador = async (req, res) => {
  ...
  try {
    ...
    const error = new Error(
      "Error del servidor al actualizar el Administrador",
    );
    return res.status(500).json({
      msg: error.message,
    });
...
const dashboard = async (req, res) => {
  ...
  try {
    ...
    const clientesInactivos = await Cliente.scope(
      "eliminarInfo",
    ).findAndCountAll({
      include: [
        {
    ...
    const sucursalesConClientes = sucursales.rows.map((sucursal) => {
      const clientesDeSucursal = clientes.rows.filter(
        (cliente) => cliente.sucursal.id === sucursal.id,
      );

      const activosDeSucursal = clientesActivos.rows.filter(
        (cliente) => cliente.sucursal.id === sucursal.id,
      );

      const inactivosDeSucursal = clientesInactivos.rows.filter(
        (cliente) => cliente.sucursal.id === sucursal.id,
      );
...
////VERIFICAR TOKEN///
const verificarSuperAdmin = async (req, res) => {
  return res.status(200).json({
    valid: true,
    data: {
      usuario: req.administrador,
    }
  });
};
