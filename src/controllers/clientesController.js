...
const crearCliente = async (req, res) => {
  ...
  try {
    ...
    const clienteRespuesta = await Cliente.scope("eliminarInfo").findByPk(
      cliente.id,
    );
...
const actualizarCliente = async (req, res) => {
  ...
  try {
    ...
    //VALIDAR SI ALGÚN CAMPO CAMBIO
    const hasChanges = Object.entries(req.body).some(
      ([key, value]) => String(cliente.get(key)) !== String(value),
    );

    console.log(hasChanges);
    if (!hasChanges) {
      return res.status(400).json({
        msg: "No se detectaron cambios para actualizar",
      });
    }
